...
(defmacro backquote (x) (bq-completely-process x))
...
(defun bq-process (x)
  (cond ((atom x) (list *bq-quote* x))
        ...
        (t
         (do ((p x (cdr p)) (q 'nil (cons (bracket (car p)) q)))
             ((atom p)
              (cons *bq-append*
...
(defun bracket (x)
  (cond ((atom x) (list *bq-list* (bq-process x)))
        ((eq (car x) *comma*) (list *bq-list* (cadr x)))
...
(defun maptree (fn x)
  (if (atom x)
    ...
    (let ((a (funcall fn (car x))) (d (maptree fn (cdr x))))
      (if (and (eql a (car x)) (eql d (cdr x))) x (cons a d)))))

(defun bq-splicing-frob (x)
  (and (consp x)
       (or (eq (car x) *comma-atsign*) (eq (car x) *comma-dot*))))
...
(defun bq-simplify (x)
  (if (atom x)
      x
    (let ((x (if (eq (car x) *bq-quote*) x (maptree #'bq-simplify x))))
      (if (not (eq (car x) *bq-append*)) x (bq-simplify-args x)))))
  ...
  (do ((args (reverse (cdr x)) (cdr args))
       (result nil
        (cond ((atom (car args))
              ...
              ((and (eq (caar args) *bq-list**)
                    (notany #'bq-splicing-frob (cdar args)))
               (bq-attach-conses (reverse (cdr (reverse (cdar args))))
              ...
              ((and (eq (caar args) *bq-quote*)
                    ...
                    (null (cddar args)))
               (bq-attach-conses (list (list *bq-quote* (caadar args)))
                                 result))
